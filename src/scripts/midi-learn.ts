...
function exportParamMap(): void {
  ...
  try {
    ...
    console.log(`\n📁 Exported param-map skeleton to: ${filePath}`);
  } catch (err) {
    console.error(`\n❌ Failed to export param-map:`, err);
  }
}

async function main() {
  console.log('\n🎓 Patchwork MIDI Learn Mode\n');
  ...
  if (portIndex === -1) {
    portIndex = 0;
    console.log(`\n⚠️  MicroFreak not found, using: "${ports[0]}"`);
  } else {
    console.log(`\n✓ Found: "${ports[portIndex]}"`);
  }
  ...
  input.on('message', (_deltaTime: number, message: number[]) => {
    ...
    if ((status & 0xf0) === 0xb0) {
      ...
      if (isNew) {
        console.log(`✨ NEW  ${formatCC(msg)}`);
      } else {
        // Overwrite same line for updates
        process.stdout.write(`\r   →   ${formatCC(msg)}    `);
      }
    }
    
    // Also log note messages for reference
    if ((status & 0xf0) === 0x90 && data2 > 0) {
      console.log(`\n🎹 Note On: ${data1} vel ${data2}`);
    }
  });
  ...
  process.on('SIGINT', () => {
    ...
    console.log('\n📋 CC Summary - Discovered mappings:\n');
